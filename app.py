...
def setup_database_indexes():
    """Ensure indexes are set up on MongoDB collections for optimal query performance"""
    try:
...
# setup_database_indexes() - Commented out for Vercel. Now run on cron.
...
def normalize_bill_datetime(raw_value):
    ...
    if isinstance(raw_value, str):
        ...
        normalized = text.replace(
            "Z", "+00:00") if text.endswith("Z") else text
        try:
...
def cleanup_old_bills():
    """Delete bills older than 6 months automatically using direct DB filters"""
    try:
...
def daily_cleanup_scheduler():
    """Runs check_and_cleanup every 24 hours."""
    while True:
        try:
            ...
            time.sleep(3600)  # Sleep for an hour on error before retrying
...
def not_found(error):
    ...
    # Serve index.html as a fallback for any unmatched paths (highly compatible with direct browser refreshes)
    try:
...
def validate_bill_data(data):
    ...
    valid_order_types = ["Dine-in", "Take Out",
                         "Dine-in / Take Out", "Zomato", "Swiggy", "Swiggy / Zomato"]
...
def add_header(r):
    """
    Prevent browser from caching API/HTML responses.
...
@app.route("/")
def home():
    """Serve main POS system from the root workspace folder"""
    try:
        return send_from_directory(".", "index.html")
    except Exception as e:
...
def check_and_reset_daily_counter():
    ...
    try:
        ...
        if last_reset_local.date() < now_local.date():
            ...
            logger.info(
                "♻️ Midnight Bill Counter Reset - Next Bill #1 (12:00 AM day rollover)")
            return 1, True
...
def get_token():
    ...
    try:
        ...
        if was_reset:
            logger.info("Bill counter reset detected - Returning Bill #1")
            return jsonify({
                "success": True,
                "token": 1
            }), 200
...
def api_cron_cleanup():
    """Daily cron endpoint to clean up bills older than 6 months and ensure indexes are up-to-date."""
    try:
        deleted_count = cleanup_old_bills()
        setup_database_indexes()
        return jsonify({
            "success": True,
            "message": "Cleanup and indexing completed successfully",
...
def save_bill():
    ...
    try:
        ...
        logger.info(
            f"✅ Bill saved - Token: {bill['token']}, Total: ₹{bill['total']}, Items: {len(bill['items'])}")
...
def get_bills():
    ...
    try:
        ...
        include_deleted = request.args.get(
            "includeDeleted", default="false").lower() == "true"
...
def delete_bill(token):
    ...
    try:
        ...
        created_at_iso = (body.get("createdAtISO") or request.args.get(
            "createdAtISO") or request.args.get("createdAt") or "").strip()
...
def permanent_delete_bill(token):
    ...
    try:
        ...
        created_at_iso = str(request_data.get("createdAtISO") or request.args.get(
            "createdAtISO") or request.args.get("createdAt") or "").strip()
...
def update_custom_item(name):
    ...
    try:
        ...
        logger.info(
            f"✅ Custom item updated: {existing_item.get('name')} -> {new_name}")
        return jsonify({
            "success": True,
...
def get_custom_categories():
    ...
    try:
        categories = list(custom_categories_col.find(
...
def delete_custom_category(name):
    ...
    try:
        ...
        removed_items = custom_items_col.delete_many(
            {"category": existing.get("name", category_name)}).deleted_count

        logger.info(
            f"✅ Custom category deleted: {existing.get('name', category_name)} (removed {removed_items} linked item(s))")
        return jsonify({
            "success": True,
...
if __name__ == "__main__":
    ...
    logger.info(
        f"🚀 Starting Flask server on port {port} (Debug: {debug_mode})")
    ...
    cleanup_thread = threading.Thread(
        target=daily_cleanup_scheduler, daemon=True, name="DailyCleanup")
